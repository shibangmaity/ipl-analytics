...
.nav-label {{ font-size:11px; color:#8899aa; font-weight:700; letter-spacing:2px; padding: 0 4px; text-transform:uppercase; }}
...
.style-card{{background:#07111e;border-radius:12px;padding:1rem;border:.5px solid #1a2a40;text-align:center;margin-bottom:8px;}}
.style-val{{font-size:22px;font-weight:900;}}
.style-lbl{{font-size:10px;color:#8899aa;margin-top:4px;}}
</style>
...
<div class="top-header">
    <div class="brand-wrap">
        <img src="{IPL_LOGO}" width="56" height="56"
             style="border-radius:50%;box-shadow:0 0 24px rgba(255,215,0,0.7);
...
def load_and_train():
    ...
    # Phase analytics
    deliveries['phase'] = pd.cut(deliveries['over'], bins=[-1,5,14,19],
    ...
    dismissal_stats = dismissal_stats[~dismissal_stats['dismissal_kind'].isin(
        ['retired hurt','obstructing the field','retired out'])]

    # Detailed bowl style wickets
    bowl_style_merge = deliveries.merge(
        players[['player_name','bowl_style']], left_on='bowler', right_on='player_name', how='left')
    wk_style = bowl_style_merge[bowl_style_merge['is_wicket']==True]
    bowl_style_stats = wk_style.groupby('bowl_style')['is_wicket'].count().reset_index()
    bowl_style_stats.columns = ['bowl_style','wickets']
    bowl_style_stats = bowl_style_stats[
        bowl_style_stats['bowl_style'].str.strip() != ''].sort_values('wickets',ascending=False).head(10)

    # City stats
    city_stats = matches.groupby('city').agg(
        matches=('id','count')).reset_index().sort_values('matches',ascending=False).head(10)

    # Allrounders
    bat_runs = deliveries.groupby('batter')['batsman_runs'].sum().reset_index()
    bat_runs.columns = ['player','runs']
    bowl_wk = deliveries[deliveries['is_wicket']==True].groupby('bowler')['is_wicket'].count().reset_index()
    bowl_wk.columns = ['player','wickets']
    allrounders_df = bat_runs.merge(bowl_wk, on='player')
    allrounders_df = allrounders_df[
        (allrounders_df['runs']>=500) & (allrounders_df['wickets']>=30)
    ].sort_values('wickets',ascending=False).head(10)

    # Arm matchup
    arm_matchup = deliveries[deliveries['bowler_type']!='Unknown'].groupby(
        ['bowler_arm','bowler_type','batter_hand']).agg(
        avg_runs=('batsman_runs','mean'),
    ...
    # Win margin analysis
    win_runs = matches[matches['result_margin']>0].sort_values('result_margin',ascending=False).head(10)
    biggest_wins = win_runs[['season','team1','team2','winner','result_margin','venue']].copy()

    return (rf,le,ve,matches,tw,tm2,acc,bat,bowl,oc,pc,season_avg,td,team_bnd,
            deliveries,players,bat_hand_stats,bowl_type_stats,
            phase_stats,dismissal_stats,bowl_style_stats,city_stats,
            allrounders_df,arm_matchup,biggest_wins)
...
with st.spinner("🏏 Loading model & stats (2008–2026)..."):
    (rf,le,ve,matches,tw,tm2,acc,bat,bowl,oc,pc,season_avg,td,team_bnd,
     deliveries,players,bat_hand_stats,bowl_type_stats,
     phase_stats,dismissal_stats,bowl_style_stats,city_stats,
     allrounders_df,arm_matchup,biggest_wins)=load_and_train()
...
if st.session_state.page=='predict':
    ...
    st.markdown('<div class="page-sub">Predict. Analyze. Win. — Updated with 2026 season data</div>',unsafe_allow_html=True)
    with st.container():
...
# ══════════════════════════════════════════════════════════════
# PAGE: STYLE ANALYTICS (NEW)
# ══════════════════════════════════════════════════════════════
elif st.session_state.page=='style':
    st.markdown('<div class="page-title">🎯 <span>Style Analytics</span></div>',unsafe_allow_html=True)
    st.markdown('<div class="page-sub">Left vs Right hand batters · Pace vs Spin bowlers · Matchup analysis</div>',unsafe_allow_html=True)

    # Top metrics
    left_pct=round((players['bat_hand']=='Left').mean()*100)
    right_pct=100-left_pct
    pace_pct=round((players['bowl_type']=='Pace').mean()*100)
    spin_pct=round((players['bowl_type']=='Spin').mean()*100)

    m1,m2,m3,m4=st.columns(4)
    for col,lbl,val,color in [
        (m1,f"Left hand batters",f"{left_pct}%","#1a6fd4"),
        (m2,f"Right hand batters",f"{right_pct}%","#f4820a"),
        (m3,f"Pace bowlers",f"{pace_pct}%","#e63946"),
        (m4,f"Spin bowlers",f"{spin_pct}%","#1c8b6e")]:
        with col: st.markdown(f'<div class="metric-card"><div class="metric-val" style="color:{color}">{val}</div><div class="metric-lbl">{lbl}</div><div class="metric-sub">of 799 players</div></div>',unsafe_allow_html=True)

    st.markdown("<br>",unsafe_allow_html=True)
    tab=st.radio("View",["🤚 Bat Hand","🏏 Bowl Type","🔥 Matchups","👤 Player Profile"],horizontal=True,key="stab",label_visibility="collapsed")

    if tab=="🤚 Bat Hand":
        st.markdown('<div class="card">',unsafe_allow_html=True)
        st.markdown('<div class="divider">LEFT HAND vs RIGHT HAND BATTER PERFORMANCE</div>',unsafe_allow_html=True)
        c1,c2=st.columns(2)
        with c1:
            for _,row in bat_hand_stats.iterrows():
                ...
                st.markdown(f"""<div style="background:#07111e;border-radius:12px;padding:1.2rem;margin-bottom:10px;border:.5px solid #1a2a40">
                    <div style="font-size:14px;font-weight:800;color:{color}">{row['batter_hand']} Hand</div>
        ...
        with c2:
            # Top left hand batters
            lhb=deliveries[deliveries['batter_hand']=='Left'].groupby('batter')['batsman_runs'].sum().reset_index()
            lhb=lhb.sort_values('batsman_runs',ascending=False).head(8).reset_index(drop=True)
        ...
        st.markdown('<div class="card">',unsafe_allow_html=True)
        st.markdown('<div class="divider">PACE vs SPIN EFFECTIVENESS</div>',unsafe_allow_html=True)
        c1,c2=st.columns(2)
        with c1:
        ...
        with c2:
            ...
            pace_wk=deliveries[(deliveries['bowler_type']=='Pace')&(deliveries['is_wicket']==True)&(~deliveries['dismissal_kind'].isin(['run out','retired hurt']))]
            pw=pace_wk.groupby('bowler')['is_wicket'].count().reset_index().sort_values('is_wicket',ascending=False).head(8).reset_index(drop=True)
        ...
        st.markdown('<div class="card">',unsafe_allow_html=True)
        st.markdown('<div class="divider">BATTER HAND vs BOWLER TYPE MATCHUP</div>',unsafe_allow_html=True)
        ...
        th='<table class="stat-table"><thead><tr><th>Batter Hand</th><th>Bowler Type</th><th>Avg Runs/Ball</th><th>Strike Rate</th><th>Wicket %</th><th>Total Balls</th></tr></thead><tbody>'
        for _,r in matchup.iterrows():
            hand_color='#1a6fd4' if r['batter_hand']=='Left' else '#f4820a'
            bowl_color='#e63946' if r['bowler_type']=='Pace' else '#1c8b6e'
            th+=f"""<tr>
                <td><span class="badge" style="background:{hand_color}22;color:{hand_color}">{r['batter_hand']}</span></td>
                <td><span class="badge" style="background:{bowl_color}22;color:{bowl_color}">{r['bowler_type']}</span></td>
                <td style="color:#fff;font-weight:700">{r['avg_runs']:.3f}</td>
                <td style="color:#f4820a;font-weight:700">{r['sr']}</td>
                <td style="color:#e63946">{r['wicket_pct']}%</td>
                <td style="color:#607080">{int(r['total_balls']):,}</td>
            </tr>"""
        th+='</tbody></table>'
        st.markdown(th,unsafe_allow_html=True)

        st.markdown('<div class="divider" style="margin-top:1rem">KEY INSIGHT</div>',unsafe_allow_html=True)
        st.markdown("""<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px">
            <div style="background:#07111e;border-radius:10px;padding:1rem;border:.5px solid #1a2a40">
                <div style="font-size:11px;font-weight:700;color:#f4820a">🏏 Batting</div>
                <div style="font-size:12px;color:#ccd;margin-top:6px;line-height:1.8">
                Left hand batters score slightly higher against pace bowling.
                Right hand batters perform more consistently across both types.
                </div>
            </div>
            <div style="background:#07111e;border-radius:10px;padding:1rem;border:.5px solid #1a2a40">
                <div style="font-size:11px;font-weight:700;color:#e63946">🎯 Bowling</div>
                <div style="font-size:12px;color:#ccd;margin-top:6px;line-height:1.8">
                Pace bowling takes more total wickets. Spin bowling is more economical.
                Left arm pace is the most dangerous bowling category.
                </div>
            </div>
        </div>""",unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
        ...
        search=st.text_input("Enter player name",placeholder="e.g. V Kohli, MS Dhoni, JJ Bumrah",label_visibility="collapsed")
        if search:
            p_match=players[players['player_name'].str.contains(search,case=False,na=False)]
            if len(p_match)>0:
                for _,p in p_match.head(3).iterrows():
                    b_stats=bat[bat['batter']==p['player_name']]
                    bw_stats=bowl[bowl['bowler']==p['player_name']]
                    bat_color='#1a6fd4' if p['bat_hand']=='Left' else '#f4820a'
                    bowl_color='#e63946' if p['bowl_type']=='Pace' else '#1c8b6e'
                    st.markdown(f"""<div style="background:#07111e;border-radius:12px;padding:1.2rem;margin-bottom:10px;border:.5px solid #1a2a40">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
                            <div style="width:44px;height:44px;border-radius:50%;background:#1a2a40;display:flex;align-items:center;justify-content:center;font-size:18px">🏏</div>
                            <div>
                                <div style="font-size:15px;font-weight:800;color:#fff">{p['player_full_name']}</div>
                                <div style="font-size:11px;color:#607080">{p['player_name']}</div>
                            </div>
                        </div>
                        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">
                            <span class="badge" style="background:{bat_color}22;color:{bat_color}">🤚 {p['bat_hand']} Hand Bat</span>
                            <span class="badge" style="background:{bowl_color}22;color:{bowl_color}">🎯 {p['bowl_style'] if pd.notna(p['bowl_style']) else 'N/A'}</span>
                        </div>""",unsafe_allow_html=True)
                    if len(b_stats)>0:
                        r=b_stats.iloc[0]
                    ...
                    if len(bw_stats)>0 and bw_stats.iloc[0]['wickets']>0:
                        r=bw_stats.iloc[0]
                ...
                st.info("No player found. Try a different name.")
        else:
            st.markdown('<div style="color:#607080;font-size:13px;text-align:center;padding:2rem">Search any player to see their batting style, bowling style, and career stats.</div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    if chart_tab=="Runs trend":
        ...
        abbr_map={"Chennai Super Kings":"CSK","Mumbai Indians":"MI","Royal Challengers Bengaluru":"RCB",
                  "Kolkata Knight Riders":"KKR","Delhi Capitals":"DC","Rajasthan Royals":"RR",
                  "Punjab Kings":"PBKS","Sunrisers Hyderabad":"SRH","Gujarat Titans":"GT","Lucknow Super Giants":"LSG"}
        names=[abbr_map.get(x[0],x[0]) for x in active_tw]
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="page-sub">Phase analytics · Dismissals · Bowl styles · Allrounders · City stats</div>',unsafe_allow_html=True)

    deep_tab=st.radio("",["⚡ Phase","🎯 Dismissals","🏏 Bowl Styles","🌟 Allrounders","🏙️ Cities","💪 Arm Matchup"],
                      horizontal=True,key="dtab",label_visibility="collapsed")
    ...
    if deep_tab=="⚡ Phase":
        ...
        for _,row in phase_stats.iterrows():
            ...
            st.markdown(f"""<div style="background:#07111e;border-radius:12px;padding:1.2rem;
                ...
                <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:10px">
                    <div style="text-align:center;background:#0c1828;border-radius:8px;padding:8px">
                        <div style="font-size:18px;font-weight:900;color:#fff">{int(row['runs']):,}</div>
                        <div style="font-size:9px;color:#607080">Total Runs</div></div>
                    <div style="text-align:center;background:#0c1828;border-radius:8px;padding:8px">
                        <div style="font-size:18px;font-weight:900;color:{color}">{row['run_rate']}</div>
                        <div style="font-size:9px;color:#607080">Run Rate</div></div>
                    <div style="text-align:center;background:#0c1828;border-radius:8px;padding:8px">
                        <div style="font-size:18px;font-weight:900;color:#fff">{int(row['wickets']):,}</div>
                        <div style="font-size:9px;color:#607080">Wickets</div></div>
                    <div style="text-align:center;background:#0c1828;border-radius:8px;padding:8px">
                        <div style="font-size:18px;font-weight:900;color:#e63946">{row['wicket_rate']}%</div>
                        <div style="font-size:9px;color:#607080">Wicket Rate</div></div>
                </div></div>""",unsafe_allow_html=True)

        st.markdown('<div class="divider" style="margin-top:1rem">KEY INSIGHTS</div>',unsafe_allow_html=True)
        st.markdown("""<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
            <div style="background:#07111e;border-radius:10px;padding:1rem;border-left:3px solid #1a6fd4">
                <div style="color:#1a6fd4;font-weight:700;font-size:11px">⚡ POWERPLAY</div>
                <div style="color:#ccd;font-size:12px;margin-top:6px;line-height:1.8">
                Run rate: 7.28<br>Lowest wicket risk<br>Set the foundation</div></div>
            <div style="background:#07111e;border-radius:10px;padding:1rem;border-left:3px solid #f4820a">
                <div style="color:#f4820a;font-weight:700;font-size:11px">🏏 MIDDLE OVERS</div>
                <div style="color:#ccd;font-size:12px;margin-top:6px;line-height:1.8">
                Run rate: 7.34<br>Most wickets taken<br>Build partnerships</div></div>
            <div style="background:#07111e;border-radius:10px;padding:1rem;border-left:3px solid #e63946">
                <div style="color:#e63946;font-weight:700;font-size:11px">💀 DEATH OVERS</div>
                <div style="color:#ccd;font-size:12px;margin-top:6px;line-height:1.8">
                Run rate: 9.03<br>Highest wicket rate 8%<br>Most explosive phase</div></div>
        </div>""",unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

    elif deep_tab=="🎯 Dismissals":
        st.markdown('<div class="card">',unsafe_allow_html=True)
        ...
        for i,(_,row) in enumerate(dismissal_stats.iterrows()):
            pct = round(row['count']/total_wk*100,1)
            color = colors[i % len(colors)]
            bar_width = pct/dismissal_stats['count'].max()*100*pct/100
            st.markdown(f"""<div class="player-row">
                <div class="player-rank">#{i+1}</div>
                <div style="flex:1">
                    <div class="player-name">{str(row['dismissal_kind']).title()}</div>
                    <div style="background:#1a2a40;height:6px;border-radius:3px;margin-top:5px">
                        <div style="width:{pct}%;height:100%;border-radius:3px;background:{color}"></div>
                    </div>
                </div>
                <div style="text-align:right">
                    <div class="player-val" style="color:{color}">{pct}%</div>
                    <div style="font-size:9px;color:#607080">{int(row['count']):,} dismissals</div>
                </div></div>""",unsafe_allow_html=True)
        st.markdown(f"""<div style="background:#07111e;border-radius:10px;padding:1rem;margin-top:1rem;
                        border:.5px solid #1a2a40;text-align:center">
            <div style="color:#607080;font-size:11px">TOTAL WICKETS ANALYZED</div>
            <div style="font-size:24px;font-weight:900;color:#f4820a">{total_wk:,}</div>
        </div>""",unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

    elif deep_tab=="🏏 Bowl Styles":
        st.markdown('<div class="card">',unsafe_allow_html=True)
        st.markdown('<div class="divider">DETAILED BOWLING STYLE — WICKETS TAKEN</div>',unsafe_allow_html=True)
        style_colors = {
            'Right arm Fast':'#e63946','Right arm Medium':'#f4820a',
            'Right arm Fast medium':'#ff6b6b','Right arm Offbreak':'#1c8b6e',
            'Slow Left arm Orthodox':'#1a6fd4','Legbreak Googly':'#a855f7',
            'Right arm Medium fast':'#f97316','Left arm Fast medium':'#00b4d8',
            'Left arm Medium fast':'#06b6d4','Legbreak':'#8b5cf6'
        }
        max_wk = bowl_style_stats['wickets'].max()
        for _,row in bowl_style_stats.iterrows():
            color = style_colors.get(str(row['bowl_style']),'#607080')
            pct = row['wickets']/max_wk*100
            st.markdown(f"""<div class="player-row">
                <div style="flex:1">
                    <div class="player-name">{row['bowl_style']}</div>
                    <div style="background:#1a2a40;height:5px;border-radius:3px;margin-top:5px">
                        <div style="width:{pct:.0f}%;height:100%;border-radius:3px;background:{color}"></div>
                    </div>
                </div>
                <div class="player-val" style="color:{color}">{int(row['wickets']):,} wkts</div>
            </div>""",unsafe_allow_html=True)
        st.markdown('<div class="divider" style="margin-top:1rem">INSIGHT</div>',unsafe_allow_html=True)
        st.markdown("""<div style="color:#94a3b8;font-size:12px;line-height:2">
            ✅ Right arm Medium pace takes most wickets overall<br>
            ✅ Right arm Fast is most lethal — highest wicket rate<br>
            ✅ Slow Left arm Orthodox (finger spin) most economical<br>
            ✅ Legbreak Googly (wrist spin) most unpredictable
        </div>""",unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

    elif deep_tab=="🌟 Allrounders":
        st.markdown('<div class="card">',unsafe_allow_html=True)
        ...
        for i,(_,row) in enumerate(allrounders_df.iterrows(),1):
            ...
            th += f"""<tr>
                <td style="color:#607080">{i}</td>
                <td style="color:#fff;font-weight:700">{row['player']}</td>
                <td style="color:#1a6fd4;font-weight:700">{int(row['runs']):,}</td>
                <td style="color:#e63946;font-weight:700">{int(row['wickets'])}</td>
                <td><span class="badge" style="background:#f4820a22;color:#f4820a">{score}</span></td>
            </tr>"""
        th += '</tbody></table>'
        st.markdown(th,unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

    elif deep_tab=="🏙️ Cities":
        st.markdown('<div class="card">',unsafe_allow_html=True)
        st.markdown('<div class="divider">CITY-WISE IPL MATCH DISTRIBUTION</div>',unsafe_allow_html=True)
        max_m = city_stats['matches'].max()
        city_colors = ['#1a6fd4','#f4820a','#e63946','#1c8b6e','#a855f7','#00b4d8','#f97316','#8b5cf6','#06b6d4','#ff6b6b']
        for i,(_,row) in enumerate(city_stats.iterrows()):
            pct = row['matches']/max_m*100
            color = city_colors[i % len(city_colors)]
            st.markdown(f"""<div class="player-row">
                <div class="player-rank">#{i+1}</div>
                <div style="flex:1">
                    <div class="player-name">🏙️ {row['city']}</div>
                    <div style="background:#1a2a40;height:6px;border-radius:3px;margin-top:5px">
                        <div style="width:{pct:.0f}%;height:100%;border-radius:3px;background:{color}"></div>
                    </div>
                </div>
                <div class="player-val" style="color:{color}">{int(row['matches'])} matches</div>
            </div>""",unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

    elif deep_tab=="💪 Arm Matchup":
        st.markdown('<div class="card">',unsafe_allow_html=True)
        st.markdown('<div class="divider">BOWLING ARM + TYPE vs BATTER HAND — DETAILED MATCHUP</div>',unsafe_allow_html=True)
        th = '<table class="stat-table"><thead><tr><th>Bowl Arm</th><th>Bowl Type</th><th>Bat Hand</th><th>Avg Runs/Ball</th><th>Strike Rate</th><th>Wicket %</th><th>Balls</th></tr></thead><tbody>'
        for _,row in arm_matchup.sort_values('wicket_pct',ascending=False).iterrows():
            arm_color = '#1a6fd4' if row['bowler_arm']=='Left' else '#f4820a'
            type_color = '#e63946' if row['bowler_type']=='Pace' else '#1c8b6e'
            hand_color = '#a855f7' if row['batter_hand']=='Left' else '#00b4d8'
            th += f"""<tr>
                <td><span class="badge" style="background:{arm_color}22;color:{arm_color}">{row['bowler_arm']}</span></td>
                <td><span class="badge" style="background:{type_color}22;color:{type_color}">{row['bowler_type']}</span></td>
                <td><span class="badge" style="background:{hand_color}22;color:{hand_color}">{row['batter_hand']}</span></td>
                <td style="color:#fff;font-weight:700">{row['avg_runs']:.3f}</td>
                <td style="color:#f4820a;font-weight:700">{row['sr']}</td>
                <td style="color:#e63946">{row['wicket_pct']}%</td>
                <td style="color:#607080">{int(row['balls']):,}</td>
            </tr>"""
        th += '</tbody></table>'
        st.markdown(th,unsafe_allow_html=True)
        st.markdown('<div class="divider" style="margin-top:1rem">KEY FINDING</div>',unsafe_allow_html=True)
        st.markdown("""<div style="color:#94a3b8;font-size:12px;line-height:2;padding:0.5rem">
            ✅ Left arm Pace is the most dangerous bowling category<br>
            ✅ Right hand batters struggle more vs Left arm pace<br>
            ✅ Wrist spin (Legbreak) is hardest to read for all batters<br>
            ✅ Death overs: Left arm pace vs Right hand batter = highest wicket %
        </div>""",unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
    ...
    <div class="card" style="margin-top:1rem"><div class="divider">PROJECT</div>
        <div style="color:#ccd;font-size:13px;line-height:2">
            ...
            <b style="color:#f4820a">1,212 matches</b>, <b style="color:#f4820a">288,226 deliveries</b>,
            and <b style="color:#f4820a">799 players</b>.<br><br>
            Uses a <b style="color:#f4820a">Random Forest Classifier</b> with data augmentation achieving
            <b style="color:#1c8b6e">{round(acc*100,1)}% accuracy</b> on held-out test set.<br><br>
            New in this version: <b style="color:#a855f7">Player Style Analytics</b> — batting hand, bowling type,
            and matchup analysis across all 799 IPL players.
        </div></div>
    <div class="card"><div class="divider">KEY FINDINGS</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;color:#aab;font-size:12px;line-height:2">
            ...
            <div>✅ Field-first preference grew significantly since 2008</div>
            <div>✅ Average match scores rose 27%+ from 2009 to 2026</div>
            <div>✅ Left hand batters slightly outperform vs pace</div>
            <div>✅ Pace bowling dominates in wicket-taking</div>
        </div></div>
    <div class="card"><div class="divider">TECH STACK</div>
        <div style="display:flex;flex-wrap:wrap;gap:8px">
            {''.join([f'<span class="badge" style="background:{c[1]}22;color:{c[1]};padding:6px 14px">{c[0]}</span>' for c in [("Python","#f4820a"),("Scikit-learn","#1a6fd4"),("Pandas","#e63946"),("Streamlit","#1c8b6e"),("Matplotlib","#a855f7"),("NumPy","#00b4d8"),("Random Forest","#f4820a"),("HuggingFace","#ffcc00")]])}
        </div></div>
    <div class="card"><div class="divider">BUILT BY</div>
        <div style="color:#ccd;font-size:13px;line-height:2">
            <b style="color:#fff">Shibang Maity</b> · Computer Science, KIIT University · 2026<br>
            <span style="color:#607080">github.com/shibangmaity · linkedin.com/in/shibangmaity</span>
        </div></div>""",unsafe_allow_html=True)

st.markdown('<div class="footer">Built by Shibang Maity · IPL Data 2008–2026 · Random Forest Classifier + Player Style Analytics</div>',unsafe_allow_html=True)