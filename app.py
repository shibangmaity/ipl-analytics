...
TEAM_INFO = {
    ...
    "Punjab Kings":                {"abbr":"PBKS", "color":"#E31E24","bg":"#0A2348","text":"#ffffff","shape":"lion"},
...
def load_and_train():
    ...
    feats=['team1_enc','team2_enc','team1_won_toss','toss_win_bat',
           'h2h_winrate','team1_form','team2_form','form_diff',
           'team1_form5','team2_form5','form5_diff',
           'venue_enc','venue_wr_diff','wr_diff',
           'season_num','season_stage',
           'team1_overall_wr','team2_overall_wr']

    X,y=matches[feats],matches['team1_won']
    split=int(len(matches)*0.75)  # 75/25 split — more training data
    rf=RandomForestClassifier(n_estimators=1000,max_depth=6,min_samples_leaf=8,max_features='sqrt',random_state=42)
    rf.fit(X[:split],y[:split]); acc=rf.score(X[split:],y[split:])
...
def predict_match(rf,le,ve,matches,tw,tm2,t1,t2,venue,t1_toss,dec):
    ...
    v=ve.transform([venue])[0] if venue in ve.classes_ else 0
    e1,e2=le.transform([t1])[0],le.transform([t2])[0]
    ...
    X=[[e1,e2,t1_toss,1 if dec=='bat' else 0,
        h2hwr,f1,f2,f1-f2,
        f1_5,f2_5,f1_5-f2_5,
        v,vwr1-vwr2,wr1-wr2,
        2024,1.0,wr1,wr2]]
