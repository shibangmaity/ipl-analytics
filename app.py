...
TEAM_INFO = {
    ...
    "Punjab Kings":                {"abbr":"PBKS", "color":"#A0001A","bg":"#1a0000","text":"#ffffff","shape":"lion"},
...
.menu-fab{position:fixed;top:12px;left:12px;z-index:9999;width:42px;height:42px;background:#f4820a;border-radius:10px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;cursor:pointer;box-shadow:0 4px 15px rgba(244,130,10,.5);transition:all .2s;}
.menu-fab:hover{background:#d4700a;transform:scale(1.08);}
.menu-fab span{display:block;width:18px;height:2px;background:#fff;border-radius:2px;}
</style>
""", unsafe_allow_html=True)

st.markdown("""
<div class="menu-fab" onclick="(function(){var b=window.parent.document.querySelector('[data-testid=\"collapsedControl\"]');if(b)b.click();})()" title="Toggle sidebar menu">
...
def load_and_train():
    try:
        ...
        st.error(f"❌ CSV not found: {e}. Place matches.csv and deliveries.csv in the same folder.")
        st.stop()
